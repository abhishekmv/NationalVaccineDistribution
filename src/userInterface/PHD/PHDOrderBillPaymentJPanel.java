 ...
 * @author Abhishek
 */
public class PHDOrderBillPaymentJPanel extends javax.swing.JPanel {
     ...
     */
    public PHDOrderBillPaymentJPanel(JPanel upc,UserAccount userAcc,EcoSystem system,PHDEnterprise pHDEnterprise) {
        initComponents();
        this.upc = upc;
        ...
        for(WorkRequest req : userAcc.getWorkQueue().getWorkRequestList()){
            if(req.getOrder()!=null)
            {
                ...
                if(req.getSender().equals(userAcc)){
                    if(req.getOrder().getPHDpaymentStatus().equals("NO")){
                        Object[] row = new Object[4];
                        row[0] = req.getOrder().getOrderNumber();
                        row[1] = getAmountDue(req);
    ...
    private float getAmountDue(WorkRequest request)
    {
        ...
        for(OrderItem oi : request.getOrder().getOrderItemList())
        {
            for(Batch b : oi.getBatchList())
            {
                ...
                {
                    if(v.getVaccineType().getFundingInfo().equals(VaccineType.NONFUNDED))
                    {
                ...
                for(WorkRequest req : userAcc.getWorkQueue().getWorkRequestList()){
                    if(req.getOrder() != null){
                        if(req.getOrder().getOrderNumber().equals(orderId)){
                            req.getOrder().setPHDpaymentStatus("YES");
                        }
                    }
                }
